...
# 데이터베이스를 페이징해서 보여줄 수 있는 함수를 선언
    ...
    def lists(self, page=0, itemsInPage=20):
    ...
    def auth(self, url, passwd):

        sql = "SELECT url , userpass "
        sql += "FROM whitelist "
        sql += "WHERE url = '{}'".format(url)
        result = False
 #       print('디버깅포인트 20', sql)
...
#            print('디버그 row1 = {}'.format(onerow))
#            print('endpoint', passwd.encode('utf-8'))

            if bcrypt.checkpw(passwd.encode("utf-8"), onerow.get('userpass').encode("utf-8")):
                result = True
                #print('디버그 row2 = {}'.format(onerow), result)
        except Exception as e:
            return("디버깅포인트  18  {}".format(e))
       # result = {} if len(result) == 0 else result[0]
        print(result)
        return result
...
# 데이터 베이스에 저장하는 함수를 선언
    ...
    def insert(self, j):
        ...
        sql = "INSERT INTO whitelist(userpass,url,reliability)"
        sql += "values('{userpass}','{url}' , '{reliability}')".format(
            userpass=self.hashpasswd(j.get("userpass", "")),
            url=utils.addslashes(json.dumps(j.get("url", ""))),
            reliability=utils.addslashes(
                json.dumps(j.get("reliability", "")))
        )
        result = None
        try:
            self.cursor.execute(sql)
